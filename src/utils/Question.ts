...
export class Question {
  ...
  constructor(data?: Question) {
    ...
    this.type = data?.type as QUESTION_TYPES || QUESTION_TYPES.RADIO;
    this.open = data?.open == false ? false : true;
    this.required = data?.required || false;
    this.answer = data?.answer || false;
    this._id = data?._id || JSON.stringify(data?._id);
    this.points = 4;
  }
  ...
  updateQuestionType(type: string): Question {
    this.type = type as QUESTION_TYPES;
  ...
  copyQuestion(): Question {
    ...
    copyQuestion._id = JSON.stringify(copyQuestion._id);
